...
- **Web app** — https://nasustim.github.io/ankiski/ is deployed by `.github/workflows/pages.yml` on every push to `main` that touches the web app or shared packages (also runnable by hand from the Actions tab). Set the repository variable `VITE_EXTENSION_ID` to the installed extension's ID so the deployed web app can talk to it; without it the web app runs standalone on IndexedDB.
- **Extension** — pushing a tag `v*` runs `.github/workflows/release.yml`, which builds `apps/extension`, zips `dist/`, and attaches it to a GitHub Release. Install it via `chrome://extensions` → Developer mode → Load unpacked after extracting the zip. Every CI run also uploads `ankiski-extension` as a workflow artifact.
